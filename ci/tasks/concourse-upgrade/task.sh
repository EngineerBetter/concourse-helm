...
helm diff upgrade "$ENV" concourse/concourse \
  ...
  --set secrets.localUsers="admin:$ADMIN_PASSWORD,devops:$DEVOPS_PASSWORD,user:$USER_PASSWORD",web.service.api.loadBalancerIP="$WEB_LB_IP",concourse.web.externalUrl="https://$WEB_DOMAIN" \
